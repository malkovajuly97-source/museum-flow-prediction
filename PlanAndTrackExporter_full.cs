...
public class PlanAndTrackExporter : MonoBehaviour
{
    ...
    [Tooltip("Запасной объект для контура пола (если Floor не даёт контур). Назначь FBX здания, напр. Nancy_Museum_unity_floor0_mirror.")]
    ...
    public void ExportAll()
    {
        ...
        try
        {
            ...
            {
                var wallOutline = GetMeshOutlineXZ(w);
                if (wallOutline != null && wallOutline.Count >= 2)
                {
                    var pts = new List<Point2D>();
                    foreach (var p in wallOutline) pts.Add(new Point2D { x = p.x, y = p.y });
