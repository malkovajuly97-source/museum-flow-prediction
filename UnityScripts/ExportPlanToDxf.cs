...
/// <summary>
/// Экспортирует геометрию пола и стен из Unity в JSON → DXF.
/// Результат: чистый план этажа без серых диагоналей.
/// Floor Transform: объект пола — экспортируется только внешний контур.
/// Walls Container: родитель со стенами. Или пусто — ищу объекты с "Wall" в имени.
/// ПКМ по скрипту → «Экспорт плана в JSON». Затем: python export_unity_plan_to_dxf.py unity_plan.json
/// </summary>
public class ExportPlanToDxf : MonoBehaviour
{
    [Header("План этажа")]
    [Tooltip("Объект пола (Floor) — меш экспортируется как внешний контур.")]
    public Transform floorTransform;

    [Tooltip("Контейнер Walls — родитель, под ним все стены. Или пусто — ищу объекты с \"Wall\" в имени.")]
    ...
    /// <summary>Извлекает только граничные рёбра меша (рёбра, принадлежащие одному треугольнику).</summary>
    ...
    /// <summary>Склеивает рёбра в замкнутые контуры.</summary>
    ...
    /// <summary>Выбирает внешний контур (наибольшая площадь), удаляет шипы, обеспечивает CCW порядок.</summary>
    ...
    public void ExportToJson()
    {
        ...
        try
        {
            ...
            Debug.Log($"ExportPlanToDxf: сохранено {path}\nЗапусти: python export_unity_plan_to_dxf.py \"{path}\"");
        }
        catch (Exception ex)
        {
            Debug.LogError($"ExportPlanToDxf: {ex.Message}");
        }
    }
